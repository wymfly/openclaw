...
**Status:** in progress under OpenSpec change `frontend-budget-hifi-contract-redesign`.

**Readiness verdict:** High after implementation. The Budget high-fidelity pass reused canonical typography, color, spacing, radius, button, form, badge/pill, status, and compact card tokens. No canonical atom/token was introduced.

**Local molecules retained:** budget metric tile, rule inventory row, selected-rule hero, threshold progress surface, scoped rule form, inline destructive confirmation, and evaluation status card.

**Repeated from prior modules:** metric tile, compact workbench header, two-column workbench rhythm, selectable row, selected evidence/detail sidecar, status/action seam, and local form sections. Budget validates these patterns for Control policy governance, but the implementation keeps them local until a dedicated design-system proposal defines shared APIs.

**Budget-specific molecules:** threshold progress surfaces, scoped budget rule forms, and guardrail evaluation status cards are candidates for a later Control policy pattern only after alerts/config/identity validate the same API shape. Real billing accuracy, usage aggregation precision, quota enforcement, and production assurance remain outside this mock/local visual pass.
...
**Status:** in progress under OpenSpec change `frontend-config-hifi-contract-redesign`.

**Readiness verdict:** High after implementation. The Config high-fidelity pass reused canonical typography, color, spacing, radius, button, form, badge/pill, code/json, status, and compact card tokens. No canonical atom/token was introduced.

**Local molecules retained:** config metric tile, hash/dirty status rail, raw JSON editor seam, schema section chip row, schema lookup surface, structured field card, sensitive-field reveal control, diff preview surface, conflict recovery strip, selected-section payload disclosure, schema lookup payload disclosure, and last-apply evidence surface.

**Repeated from prior modules:** metric tile, compact workbench header, two-column workbench rhythm, selected evidence/detail sidecar, guarded mutation preview, action/result seam, and raw payload disclosure. Config validates these patterns for governance/configuration workflows, but the implementation keeps them local until a dedicated design-system proposal defines shared APIs.

**Config-specific molecules:** raw JSON editor seams, schema lookup controls, structured field cards, sensitive display controls, diff preview surfaces, and base-hash conflict recovery strips are candidates for later Governance/Configuration patterns only after nodes/docs or another configuration-heavy module validates the same API shape. Schema authoring, schema migration generation, history/version restore, config import/export, secret vault integration, collaborative editing, and production rollback assurance remain outside this mock/local visual pass.

**Mock visual evidence:** `deck-go/test/e2e/config-visual.spec.ts` covers the ready config governance workbench, sensitive reveal, structured field edit, diff preview, and apply/hash-refresh state against bundled mock Gateway `config.get`, `config.schema.lookup`, and `config.apply`. This is mock/local visual coverage, not real Gateway/LLM, production config mutation, secret vault integration, schema migration, history restore, or rollback assurance.
