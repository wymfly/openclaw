...
| Replay / Projection Model               | A + B | 1     | P0       | Gateway Transport       | `deck-chat-flow-closure`, `session-scoped-state`, `deck-projection-platform` design                | `partial`           | Phase 1: store generalization + approval extraction + gap signal; Phase 2: HTTP endpoints, domain catch-up, transport-layer lastEventId, approvals panel migration |
