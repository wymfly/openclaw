...
| Gateway Transport / Typed Client        | A     | 1     | P0       | none                    | `gateway-protocol-sdk` design+plan, `deck-platform-hardening`                                      | `replacement-ready` | 25/25 deck.\* + 4 upstream schemas (sessions.usage/logs/timeseries, skills.install) typed; tools.effective stub; 4 routes migrated to gwRequest                                          |
...
| Shared Error / Mutation Model           | A     | 1     | P1       | Gateway Transport       | `deck-platform-hardening`                                                                          | `partial`           | unified error codes done (GatewayErrorCode + DeckApiError + fetchApi + mapGatewayError; 5 stores migrated); rollback API deferred                                                        |
| **Session Runtime**                     |       |       |          |                         |                                                                                                    |                     |                                                                                                                                                                                          |
...
| **UI Framework**                        |       |       |          |                         |                                                                                                    |                     |                                                                                                                                                                                          |
| Shared Shell / Panel Layout             | C     | 1     | P1       | none                    | `openclaw-deck`, `schema-driven-ui-architecture`, `deck-platform-hardening`                        | `replacement-ready` | PanelRegistry (23 entries) with lazy/eager, 3 consumers migrated (NavRail, page.tsx, shortcuts); registration single-source                                                              |
...
| Capability Coverage Gate                | E     | 1-5   | P0       | all tracks              | `deck-web-replacement-program`, `deck-platform-hardening`                                          | `replacement-ready` | Matrix (30 areas) + `pnpm protocol:coverage:check` automated script (141 methods: 43 typed, 36 untyped, 62 not-covered)                                                                  |
