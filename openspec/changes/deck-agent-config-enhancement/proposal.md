...
2. **技能管理只读** — SkillsTab 展示技能列表和 eligibility，但无安装/更新 UI。`skills.install`（install options + ClawHub）和 `skills.update`（配置 + 更新）API 完全未用
...
- 增强 **Skills Management**：新增技能安装 UI（install options + ClawHub slug），技能配置编辑（apiKey、env 变量），技能更新功能
...
- `agent-skills-management`: 技能安装（install options / ClawHub）、配置编辑、更新功能
