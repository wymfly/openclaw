...
- [x] 1.1 Read this proposal, design, spec deltas, current Sessions handoff
...
- [x] 1.2 Inspect current worktree status and record unrelated pre-existing
      changes so this change does not revert or overwrite them.
- [x] 1.3 Create or update a tracked Sessions convergence fact baseline in the
...
- [x] 1.4 Refresh the Sessions workflow-to-contract matrix for inventory,
...
- [x] 1.5 Add explicit ownership classifications to the matrix:
      Sessions-owned, adjacent-owned, product-local, unsupported/projected,
      empty-valid, environment-dependent, and skipped-safe.
- [x] 1.6 Classify Gateway-supported but currently BFF/product-unsurfaced
      ...
      fields.
- [x] 1.7 Confirm that create/send/abort/steer are Chat/runtime-adjacent
      workflows, not Sessions page controls, unless code truth or product
      evidence proves a Sessions-owned use case.
- [x] 1.8 Fix deterministic Sessions-scoped contract, backend, frontend, mock,
...
- [x] 2.4 Ensure the prototype keeps display strings literal, avoids new token
      requirements unless justified, and does not claim functionality outside
      the refreshed contract matrix.
- [x] 2.5 Update handoff docs so Sessions is described as browse, inspect, and
      guarded maintenance, with Chat/Subagents/Usage boundaries called out
      explicitly.
- [x] 2.6 Update handoff interactions/API notes so reset, clear, compact,
...
- [x] 3.1 Refactor `SessionsPanel` into the new information hierarchy while
      ...
      behavior.
- [x] 3.2 Implement the default-open Inspector with `Overview`, `Usage`,
      `Compaction`, `Lineage`, and `Actions` tabs using existing
      design-system primitives and accessible keyboard/focus behavior.
- [x] 3.3 Move secondary evidence and mutations into the proper Inspector tabs
      without introducing tab-triggered lazy fetches.
- [x] 3.4 Keep transcript search/export in the selected-session workbench and
      ensure export remains client-side only.
- [x] 3.5 Keep reset, clear, patch, compact, delete, branch, and restore routed
...
- [x] 3.6 Update Sessions CSS and i18n for the new hierarchy in both English
      and Chinese while preserving the normalized typography/design-system
      weight constraints.
- [x] 3.7 Extract local presentational molecules only where they directly reduce
      the Sessions component size or clarify the new layout; avoid unrelated
      cross-module pattern promotion.
- [x] 3.8 Do not add chat compose/send/abort/steer controls to Sessions; if an
...
- [x] 4.1 Update or add focused `SessionsPanel` tests for selection, filters,
      transcript cache/search/export, Inspector tab switching, usage,
      compaction, lineage, and guarded actions.
- [x] 4.2 Add focused tests proving reset, clear, compact, delete, and restore
      do not call mutation wrappers on the first click and only execute after
      confirmation.
- [x] 4.3 Update the Sessions mock visual E2E to assert the new
      list/workbench/default-open Inspector state and at least one secondary
      Inspector tab interaction.
- [x] 4.4 Run focused frontend tests for Sessions and fix deterministic
      failures before widening.
- [x] 4.5 Run the Sessions mock visual E2E and save fresh mock-functional
      evidence.
- [x] 4.6 Generate or refresh prototype-current comparison artifacts against
...
- [x] 5.1 Update real Sessions E2E so it covers Deck shell navigation into
...
- [x] 5.2 Preserve safe mutation boundaries: destructive real writes must run
      only against proven run-scoped targets or be recorded as skipped-safe /
      expected rejection evidence.
- [x] 5.3 Run bounded real Gateway Sessions evidence when the environment is
      ...
      local completion.
- [x] 5.4 Fix deterministic code defects found by real E2E directly; only
      circuit-break environment, credential, or upstream availability blockers.

## 6. Closeout And Validation

- [x] 6.1 Update Sessions implementation notes and any tracked evidence
      ...
      exceptions.
- [x] 6.2 Run `openspec validate deck-go-sessions-module-convergence --strict`.
- [x] 6.3 Run the narrow relevant frontend build/test verification and any
      contract/backend checks required by touched files.
- [x] 6.4 Mark tasks complete only after the corresponding fresh evidence
      exists.
- [x] 6.5 Confirm archive readiness after all tasks and validations pass or
