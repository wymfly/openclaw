...
The production logs panel SHALL render and operate from contract-backed log tail and stream event data while preserving the existing panel registry and API facade boundaries.

#### Scenario: Logs tail is loaded

- **WHEN** `fetchLogsTail` returns `DeckGoLogsTailResponse`
- **THEN** the panel SHALL show tail status, stream status, cursor, visible count, filters, parsed log rows, export affordance, live event tape, stream event inspection, and raw tail payload details
...
- **WHEN** an operator changes level, source, or session filters
...
- **THEN** the panel SHALL update cursor/localStorage, live tape, tail rows, and reset state using existing stream parser behavior
