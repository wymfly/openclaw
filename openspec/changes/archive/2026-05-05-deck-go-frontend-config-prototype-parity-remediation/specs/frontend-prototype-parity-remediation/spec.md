...
#### Scenario: Config row is remediated

- **WHEN** the Config child proposal archives
- **THEN** the remediation matrix SHALL record the Config active prototype,
  child proposal archive path, mock parity verdict, real Gateway evidence
  status, accepted exceptions, and final verdict
- **AND** the head task `5.7` SHALL be marked complete only after the Config
