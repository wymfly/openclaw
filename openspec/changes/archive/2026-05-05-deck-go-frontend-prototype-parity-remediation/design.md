...
### D7: Real Gateway evidence must exercise the product surface, not only RPCs

Module real E2E evidence must prove the user can reach the module through Deck
shell navigation, see it under all four supported theme/locale combinations
(`dark/en`, `dark/zh`, `light/en`, and `light/zh`) unless the child records a
specific skipped-safe reason, interact with all meaningful child pages, tabs,
dialogs, or subviews that are part of the module's product surface, and use
run-scoped real data when the Gateway/deck-go chain can create that data safely
in the isolated test state. Direct panel URLs and direct RPC checks can support
the evidence, but they cannot replace the shell-driven product flow.

For OpenClaw-backed state, child proposals should prefer Gateway RPC or Deck BFF
routes over hand-editing files, but direct isolated `openclaw.json` or workspace
seeding is acceptable when it is clearer and scoped to the temporary real E2E
state. Mutating tests must create disposable names that include the current
run id and must refuse cleanup of anything that is not run-scoped. Only
high-impact classes such as external channel accounts, installed skills, device
tokens, or user memory mutations may remain skipped-safe without fixture
creation.

For modules whose normal product value depends on configured resources,
`openclaw.json`, workspace history, sessions, or other Gateway-backed state, an
empty real page is not sufficient unless fixture creation has been explicitly
attempted and circuit-broken. The preferred fixture order is: Gateway RPC or
Deck BFF mutation first, isolated `openclaw.json` setup second, and isolated
workspace/session file setup third. Playwright evidence should validate the
user-visible flow after the fixture exists, including shell navigation, primary
list/detail surfaces, every meaningful tab/subpage/dialog that is safe to open,
and recoverable filter/search/form states. Theme and locale evidence must cover
all four combinations for normal module remediation; a reduced pair is allowed
only when the child proposal records why the remaining variants are redundant or
environment-blocked and the module has no known visual, density, or i18n drift.

Alternative considered: keep real E2E as read-only API smoke plus one direct
panel URL. Rejected because it misses product failures like navigation drift,
locale/theme regressions, live stream state bugs, and empty or unrepresentative
real data.
