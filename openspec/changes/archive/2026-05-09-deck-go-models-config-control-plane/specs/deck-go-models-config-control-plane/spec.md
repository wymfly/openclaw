...
- **THEN** deck-go SHALL support product controls for supported provider fields including `baseUrl`, `apiKey`, `auth`, `api`, `headers`, `authHeader`, `injectNumCtxForOpenAICompat`, and contained `models`
- **AND** `request` SHALL remain an advanced/raw escape hatch unless separately productized.

#### Scenario: Model fields are edited

- **WHEN** a model is created or edited
- **THEN** deck-go SHALL support product controls for supported model fields including `id`, `name`, `api`, `reasoning`, `input`, `cost`, `contextWindow`, `contextTokens`, `maxTokens`, and `headers`
- **AND** `compat` SHALL remain an advanced/raw escape hatch unless separately productized.
...
- **THEN** it SHALL render a catalog header, provider-grouped list, model rows, provider/model drawers, add-provider wizard, and impact dialogs from typed DTOs
...
- **AND** raw config save SHALL appear only in advanced escape-hatch flows.
