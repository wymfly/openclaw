...
The system SHALL detect bash/terminal tool results and render them in a split-view layout with separate sections for command, stdout, stderr, and exit code. Vite Deck SHALL match old Deck bash result visual treatment, including header affordances, monospace output, status badges, raw toggle behavior, and overflow handling.
...
The system SHALL detect file read/write tool results and render an inline diff view when both before and after content are available. Vite Deck SHALL restore old Deck diff/code preview components rather than reducing file operations to plain text.
...
The system SHALL use virtual scrolling for tool results exceeding 200 lines, replacing the current max-height truncation. Vite Deck SHALL keep the old Deck long-output interaction model, including line count, expand/collapse, and scroll behavior.
...
- **THEN** the card SHALL render all lines directly without virtual scrolling.
...
Every enhanced tool result view SHALL provide a "Show Raw" toggle that switches between the formatted view and the original unprocessed text content. Vite Deck SHALL preserve old Deck raw toggle placement and per-card state.
