...
function emitNamedSchemaType(lines: string[], name: string, schema: TSchema): void {
...
function generateProtocolTypes(): string {
  ...
  emitNamedSchemaType(lines, "TranscriptBlock", TranscriptBlockSchema);
  emitNamedSchemaType(lines, "TranscriptMessage", TranscriptMessageSchema);
  ...
  for (const [eventName, def] of Object.entries(allEventDefs)) {
    if (!def.payload) {
      continue;
    }
    const safeName = `${methodToInterfaceName(eventName)}EventPayload`;
    emitNamedSchemaType(lines, safeName, def.payload);
    eventEntries.push(`  "${eventName}": ${safeName};`);
  }
...
function generateClient(): string {
  const lines: string[] = [
    "// Auto-generated by scripts/protocol-gen-ts.ts — DO NOT EDIT",
    "",
    'import type { GatewayEventPayloadMap, GatewayMethodMap, GatewayMethodName } from "./gateway-protocol.generated";',
    "",
  ...
  for (const e of Object.keys(allEventDefs).toSorted()) {
    lines.push(`  "${e}",`);
  }
  lines.push("]);");
  lines.push("");
