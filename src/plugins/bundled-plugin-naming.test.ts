...
// [enhanced] Fork-specific packages where npmSpec intentionally differs from package name
const NPMSPEC_EXCEPTIONS = new Set(["wecom"]);
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return [
    ...
    // [enhanced] Allow @openclaw-enhanced/ scope for fork-specific packages
    ...ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@openclaw-enhanced/${pluginId}${suffix}`),
  ];
}
...
describe("bundled plugin naming guardrails", () => {
  ...
  it("keeps bundled openclaw.install.npmSpec aligned with the package name", () => {
    const mismatches = readBundledPluginRecords()
      .filter(
        ({ installNpmSpec, packageName, manifestId }) =>
          typeof installNpmSpec === "string" &&
          installNpmSpec !== packageName &&
          !NPMSPEC_EXCEPTIONS.has(manifestId),
      )
      .map(
