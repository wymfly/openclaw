...
| Send chat message and receive streamed response   |                | `make smoke-stage3-host` | partial-proof | must-match     |       | Richer smoke creates a session, sends a message through the active control-plane path, and waits until history exposes both user and assistant messages, but it does not yet assert a streamed assistant response end-to-end.        |
| Abort active chat/session run                     |                | `make smoke-stage3-host` | partial-proof | must-match     |       | Richer smoke aborts the started chat run and requires an `aborted` response from the backend.                                                                                                                                        |
