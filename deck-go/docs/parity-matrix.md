...
| Send chat message and receive streamed response   |                | `make smoke-stage3-host` | partial-proof | must-match     |       | Richer smoke creates a session and sends a message through the active control-plane path, but it does not yet assert a returned streamed assistant response.                                                                         |
