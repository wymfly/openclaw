...
| Load overview/status/inventory                    |                | `make smoke-stage3-host` | partial-proof | must-match     |       | Browser smoke hydrates the restored shell and verifies core host chrome/navigation.                                            |
...
| Read logs/basic operator observability            |                | `make smoke-stage3-host` | partial-proof | must-match     |       | Smoke proves logs route wiring plus restored shell hydration, but not richer observability interaction.                        |
