...
| Connect to Gateway and bootstrap capabilities     |                | `make smoke-stage3-host` | partial-proof | must-match     |       | Default smoke proves `/api/runtime/gateway` and `/api/bootstrap/status`; richer smoke with `DECK_GO_SMOKE_GATEWAY_TOKEN` also proves lifecycle start acceptance under the active Vite host.                |
...
| Load channels/plugins/models inventory            |                | `make smoke-stage3-host` | partial-proof | must-match     |       | Browser smoke navigates into `Channels`, `Plugins`, and `Models`; default smoke proves route wiring and richer smoke upgrades several of those routes to `200` data proof when a gateway token is present. |
| Load and save critical config                     |                | `make smoke-stage3-host` | partial-proof | must-match     |       | Smoke proves browser navigation into `Config`; richer smoke upgrades `config` / `models config` route proof to `200`, but save mutations are still not automated.                                          |
