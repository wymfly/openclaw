...
export interface MarkdownProps {
  /** Markdown source. Caller supplies — design-system makes no copy decisions. */
  content: string;
  /** Optional className appended to the wrapper. */
  className?: string;
  /** `streaming` mode marks the wrapper for live-updating content (e.g. assistant SSE deltas). */
  mode?: MarkdownMode;
}
...
type MarkdownListItem = { blocks: MarkdownBlock[] };
type ListMarker = { ordered: boolean; start: number | null; text: string };

/**
 * Text atom: minimal CommonMark-ish renderer (paragraphs, headings, lists,
 * tables, blockquotes, fenced code, links, images, inline code, emphasis).
 *
 * No external deps. Sanitized link `href` (https/http/mailto/relative only).
 * Caller styles via the wrapper class `.ds-md` and inherited tokens.
 *
 * Ported from `panels/chat/shared-renderer/MarkdownViewer.tsx` to remove
 * deck-ui-* class names and the wrapping artifact div.
 */
export function Markdown({ content, className, mode }: MarkdownProps) {
  const classes = ["ds-md"];
  if (className) {
    classes.push(className);
  }
  return (
    <div className={classes.join(" ")} data-markdown-mode={mode}>
...
function parseBlocks(lines: string[]): MarkdownBlock[] {
  ...
  while (index < lines.length) {
    ...
    if (fencedCode) {
      ...
      blocks.push({ type: "code", language: fencedCode.language, value: codeLines.join("\n") });
      continue;
    }
...
function parseTable(lines: string[], startIndex: number) {
  ...
  return { block: { type: "table", header, rows } as const, nextIndex: index };
}
