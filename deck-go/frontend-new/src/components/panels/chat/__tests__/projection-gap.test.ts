...
let clearTranscriptCache: typeof import("@/lib/transcript-cache").clearTranscriptCache;
let getCachedTranscript: typeof import("@/lib/transcript-cache").getCachedTranscript;
...
beforeEach(async () => {
  ...
  ({ clearTranscriptCache, getCachedTranscript } = await import("@/lib/transcript-cache"));
...
describe("handleProjectionGap", () => {
  it("evicts stale sessions and refetches active session snapshot", async () => {
    ...
    expect(getCachedTranscript("active-sess")).toMatchObject([
      {
        id: "active-sess:0:0",
        role: "assistant",
        content: [{ type: "text", text: "normalized" }],
