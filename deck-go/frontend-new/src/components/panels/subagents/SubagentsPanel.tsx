...
function statusPillTone(status?: string): PanelPillTone {
  if (status === "completed") {
    return "positive";
  }
  if (status === "failed") {
...
export function SubagentsPanel() {
  ...
  return (
    <PanelRoot data-testid="subagents-panel" density="compact">
      <div className="subagents-panel">
        <PanelSectionHeader
          eyebrow="operations / subagents"
          title={t("title")}
          description={t("descriptionV2")}
          actions={
        ...
        <KpiStrip columns={5} aria-label={t("runs")}>
          <PanelMetric
            label={t("visibleCount", { count: filteredRuns.length })}
            value={filteredRuns.length}
          />
          <PanelMetric label={t("serverTotal", { count: total })} value={total} />
          <PanelMetric label={t("activeStat")} value={liveCount} tone="positive" />
          <PanelMetric label={t("failedKpi")} value={failedCount} tone="warning" />
          <PanelMetric
        ...
        <div className="subagents-workbench">
          <PanelSurface>
            <div className="subagents-list-card">
              ...
              {selectedRun ? (
                <>
                  ...
                  >
                    <PanelSectionHeader
                      headingLevel={3}
                      eyebrow={t("selectedRun")}
                      title={
                        <span className="hero__title-inline">
                          <AgentGlyph
                            id={selectedRun.childAgentId}
                            name={selectedRun.childAgentName}
                          />
                          <span>{selectedRun.childAgentName || selectedRun.childAgentId}</span>
                        </span>
                      }
                      description={selectedRun.task || selectedRun.runId}
                      actions={
                        <PanelStatusRow align="end">
                          <PanelPill tone={statusPillTone(selectedRun.status)}>
                    ...
                    {detailTab === "overview" ? (
                      <>
                        <div className="field-grid">
                          ...
                          <Field
                            label={t("spawnMode")}
                            value={selectedRun.spawnMode || emptyLabel}
                          />
                        ...
                        <div className="detail-actions">
                          <Button
                            size="sm"
                            onClick={() =>
                              navigateToAgent(ui, selectedRun.childAgentId, "subagents")
                            }
                          >
                            {t("openChildAgent")}
                          </Button>
                    ...
                    {detailTab === "permissions" ? (
                      <div className="permission-grid permission-grid--readonly">
                        <div className="section__heading">
                          <div>
                            <h3>
                              {selectedRun.requesterAgentName || selectedRun.requesterAgentId}
                            </h3>
                        ...
                        <Field
                          label={t("model")}
                          value={
                            agentConfigs[selectedRun.requesterAgentId]?.model || t("inheritModel")
                          }
                        />
                        <Field
                          label={t("hash")}
                          value={
                            agentConfigs[selectedRun.requesterAgentId]?.configHash || emptyLabel
                          }
                        />
                      </div>
                    ) : null}
