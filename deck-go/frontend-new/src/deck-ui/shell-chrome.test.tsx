...
describe("Deck shell chrome parity", () => {
  ...
  it("uses workbench content mode only for chat", () => {
    uiState.activePanel = "chat";

    renderWithLocale(createElement(DeckShell, null, createElement("div", null, "Chat body")), "en");

    const content = container.querySelector(".deck-ui-content") as HTMLElement;
    expect(container.querySelector(".deck-ui-shell")?.getAttribute("data-active-panel")).toBe(
      "chat",
    );
  ...
  it("keeps normal padded content mode for non-chat panels", () => {
    ...
    renderWithLocale(
      createElement(DeckShell, null, createElement("div", null, "Gateway body")),
      "en",
    );

    const content = container.querySelector(".deck-ui-content") as HTMLElement;
    expect(content.getAttribute("data-active-panel")).toBe("gateway");
    expect(content.classList.contains("deck-ui-content--workbench")).toBe(false);
  });
