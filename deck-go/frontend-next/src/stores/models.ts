...
async function patchConfig(
  ...
  try {
    const res = await deckFetch("/api/models/config", {
      method: "PATCH",
...
export const useModelsStore = create<ModelsState>((set, get) => ({
  ...
  fetchCatalog: async () => {
    ...
    try {
      const res = await deckFetch("/api/models");
      if (!res.ok) {
        return;
      }
  ...
  fetchCatalogProviders: async () => {
    ...
    try {
      const res = await deckFetch("/api/models/catalog-providers");
      if (!res.ok) {
  ...
  fetchUsableModels: async () => {
    ...
    try {
      const res = await deckFetch("/api/models/configured");
      if (!res || !res.ok) {
      ...
      const list: Model[] = raw
        ...
        .map((m: Record<string, unknown>) => {
          ...
          return {
            ...
            editable: typeof m.editable === "boolean" ? (m.editable) : false,
            authStatus: typeof m.authStatus === "string" ? (m.authStatus) : "unknown",
          };
        });
  ...
  fetchAuthOverview: async () => {
    ...
    try {
      const res = await deckFetch("/api/models/auth");
      if (!res.ok) {
        return;
      }
      const data = await res.json();
  ...
  runProbe: async (provider: string): Promise<ProbeResult | null> => {
    try {
      const res = await deckFetch("/api/models/probe", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  fetchUsageSummary: async () => {
    try {
      const [costRes, statusRes] = await Promise.all([
        deckFetch("/api/models/usage/cost"),
        deckFetch("/api/models/usage/providers"),
      ]);
