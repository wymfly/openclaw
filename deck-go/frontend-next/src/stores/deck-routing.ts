...
export const useDeckRoutingStore = create<DeckRoutingState>((set, get) => ({
  ...
  fetchBindings: async (agentId?: string) => {
    ...
    try {
      ...
      const res = await deckFetch(`/api/deck/routing?${params.toString()}`);
      if (!res.ok) {
  ...
  addBinding: async (match, agentId, baseHash, position?) => {
    try {
      ...
      const res = await deckFetch("/api/deck/routing", {
        method: "POST",
  ...
  reorderBinding: async (binding, newPosition, baseHash) => {
    try {
      ...
      const removeRes = await deckFetch("/api/deck/routing", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const addRes = await deckFetch("/api/deck/routing", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "add",
