...
export type RuntimeGatewayActionState = "idle" | "starting" | "stopping" | "restarting";
...
interface MonitorState {
  ...
  runtimeGateway: DeckGoRuntimeGatewayStatus | null;
  runtimeGatewayLoading: boolean;
  runtimeGatewayAction: RuntimeGatewayActionState;
  runtimeGatewayError: string | null;
  ...
  fetchRuntimeGateway: () => Promise<void>;
  startRuntimeGateway: () => Promise<void>;
  stopRuntimeGateway: () => Promise<void>;
  restartRuntimeGateway: () => Promise<void>;
...
type RuntimeGatewayPayload = Partial<DeckGoRuntimeGatewayActionResponse> & { error?: string };

async function readRuntimeGatewayPayload(res: Response): Promise<RuntimeGatewayPayload | null> {
  try {
    return (await res.json()) as RuntimeGatewayPayload;
  } catch {
    return null;
  }
}

function runtimeErrorMessage(payload: RuntimeGatewayPayload | null, fallback: string): string {
  return typeof payload?.error === "string" && payload.error.trim() ? payload.error : fallback;
}

export const useMonitorStore = create<MonitorState>((set, get) => {
  const runRuntimeGatewayAction = async (
    actionState: Exclude<RuntimeGatewayActionState, "idle">,
    path: string,
  ) => {
    set({ runtimeGatewayAction: actionState, runtimeGatewayError: null });
    try {
      const res = await deckFetch(path, { method: "POST" });
      const payload = await readRuntimeGatewayPayload(res);
      if (!res.ok) {
        set({
          runtimeGateway: payload?.runtime ?? get().runtimeGateway,
          runtimeGatewayError: runtimeErrorMessage(
            payload,
            `Gateway action failed with status ${res.status}`,
          ),
        });
        return;
      }
      ...
      await Promise.allSettled([get().fetchHealth(), get().fetchStatus()]);
    } catch (error) {
      set({
        runtimeGatewayError:
          error instanceof Error ? error.message : "Failed to run gateway lifecycle action",
      });
    } finally {
      set({ runtimeGatewayAction: "idle" });
    }
  };
  ...
  // ── Runs ──
    ...
    fetchRuns: async () => {
      ...
      try {
        ...
        const res = await deckFetch(`/api/monitor/runs?${params.toString()}`);
        if (!res.ok) {
          return;
        }
  ...
  // ── Run detail ──
    ...
    try {
      const res = await deckFetch(`/api/monitor/runs/${encodeURIComponent(runId)}`);
      if (!res.ok) {
        return;
      }
  ...
  // ── Stats ──
    ...
    try {
      const res = await deckFetch("/api/monitor/stats");
      if (!res.ok) {
        return;
      }
  ...
  // ── Live events fetch (same endpoint as old activity store) ──
    ...
    try {
      const res = await deckFetch("/api/activity?limit=100");
      if (!res.ok) {
        return;
      }
  ...
  // ── Gateway diagnostics ──
    ...
    fetchRuntimeGateway: async () => {
      set({ runtimeGatewayLoading: true, runtimeGatewayError: null });
      try {
        const res = await deckFetch("/api/runtime/gateway");
        const payload = await readRuntimeGatewayPayload(res);
        if (!res.ok) {
          set({
            runtimeGateway: payload?.runtime ?? null,
            runtimeGatewayError: runtimeErrorMessage(
              payload,
              `Failed to fetch runtime gateway status (${res.status})`,
            ),
          });
          return;
        }
        ...
        set({
          runtimeGatewayError:
            error instanceof Error ? error.message : "Failed to fetch runtime gateway status",
        });
      } finally {
        set({ runtimeGatewayLoading: false });
      }
    },

    startRuntimeGateway: async () =>
      runRuntimeGatewayAction("starting", "/api/runtime/gateway/start"),

    stopRuntimeGateway: async () =>
      runRuntimeGatewayAction("stopping", "/api/runtime/gateway/stop"),

    restartRuntimeGateway: async () =>
      runRuntimeGatewayAction("restarting", "/api/runtime/gateway/restart"),

    fetchHealth: async () => {
      set({ healthLoading: true });
      try {
        const start = Date.now();
        const res = await deckFetch("/api/gateway/health");
    ...
    fetchStatus: async () => {
      ...
      try {
        const res = await deckFetch("/api/gateway/status");
        if (res.ok) {
