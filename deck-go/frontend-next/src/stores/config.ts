...
export const useConfigStore = create<ConfigState>((set, get) => ({
  ...
  fetchSchema: async () => {
    try {
      const res = await deckFetch("/api/config/schema");
      if (!res.ok) {
        return;
      }
  ...
  saveConfig: async () => {
    ...
    try {
      const res = await deckFetch("/api/config/apply", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        if (isConflict) {
          ...
          try {
            const remoteRes = await deckFetch("/api/config");
            if (remoteRes.ok) {
  ...
  lookupSchema: async (path: string) => {
    ...
    try {
      const res = await deckFetch("/api/config/schema-lookup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
