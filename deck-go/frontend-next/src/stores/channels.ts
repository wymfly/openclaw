...
export const useChannelsStore = create<ChannelsState>((set, get) => ({
  ...
  fetchChannels: async () => {
    ...
    try {
      const res = await deckFetch("/api/channels");
      if (!res.ok) {
  ...
  updateChannelConfig: async (channelId, patch) => {
    try {
      const res = await deckFetch(`/api/channels/${encodeURIComponent(channelId)}`, {
        method: "PATCH",
  ...
  logoutChannel: async (channelId) => {
    try {
      const res = await deckFetch(`/api/channels/${encodeURIComponent(channelId)}/logout`, {
        method: "POST",
      });
      if (res.ok) {
        await get().fetchChannels();
  ...
  probeChannel: async (channelId: string) => {
    ...
    try {
      const res = await deckFetch("/api/channels/probe", { method: "POST" });
