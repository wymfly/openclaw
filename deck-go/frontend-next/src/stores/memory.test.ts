...
describe("memory store", () => {
  ...
  describe("searchMemory with scope", () => {
    ...
    it("should surface not-implemented search as a degraded state instead of a generic error", async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 501,
        json: async () => ({ error: "Not implemented — requires LanceDB extension" }),
      });
      ...
      expect(state.isLanceDbEnabled).toBe(false);
      expect(state.searchUnavailableReason).toBe("Not implemented — requires LanceDB extension");
    });
  });
