...
export const useDeckAgentsStore = create<DeckAgentsState>((set, get) => ({
  ...
  fetchDetail: async (agentId: string, force = false) => {
    ...
    try {
      const res = await deckFetch(`/api/deck/agents?agentId=${encodeURIComponent(agentId)}`);
      if (!res.ok) {
  ...
  fetchSkills: async (agentId: string) => {
    try {
      const res = await deckFetch("/api/deck/agents", {
        method: "POST",
  ...
  saveBootstrapFile: async (agentId: string, name: string, content: string) => {
    try {
      const res = await deckFetch(`/api/agents/${encodeURIComponent(agentId)}/files`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  fetchAgentRawConfig: async (agentId: string) => {
    ...
    try {
      const res = await deckFetch("/api/config");
      if (!res.ok) {
        return;
      }
  ...
  saveAgentConfig: async (agentId: string, updates: Record<string, unknown>) => {
    ...
    try {
      ...
      const res = await deckFetch("/api/config/patch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  fetchToolsCatalog: async (agentId: string) => {
    ...
    try {
      const res = await deckFetch("/api/tools/catalog", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  fetchIdentity: async (agentId: string) => {
    ...
    try {
      const res = await deckFetch(`/api/agents/${encodeURIComponent(agentId)}/identity`);
      if (!res.ok) {
        set({ agentIdentity: null });
        return;
      }
