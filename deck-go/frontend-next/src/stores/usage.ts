...
export const useUsageStore = create<UsageState>((set, get) => ({
  ...
  fetchAll: async () => {
    ...
    try {
      const costRes = await deckFetch(`/api/usage/cost?days=${days}`, { signal: ac.signal });
      if (costRes.ok) {
    ...
    try {
      const sessRes = await deckFetch(`/api/usage/sessions?startDate=${startDate}&endDate=${endDate}`, {
        signal: ac.signal,
      });
      if (!sessRes.ok) {
  ...
  fetchSessionLogs: async (key) => {
    const res = await deckFetch(`/api/usage/sessions/logs?key=${encodeURIComponent(key)}`);
    if (!res.ok) {
      return [];
    }
