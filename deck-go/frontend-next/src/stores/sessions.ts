...
export const useSessionsStore = create<SessionsState>((set, get) => ({
  ...
  fetchSessions: async (opts) => {
    ...
    try {
      ...
      const res = await deckFetch(`/api/sessions${qs ? `?${qs}` : ""}`);
      if (!res.ok) {
  ...
  fetchHistory: async (sessionKey) => {
    try {
      ...
      const res = await deckFetch(`/api/chat/history?sessionKey=${encodeURIComponent(sessionKey)}`);
      if (!res.ok) {
        return;
      }
      const data = await res.json();
  ...
  deleteSession: async (sessionKey) => {
    try {
      const res = await deckFetch(`/api/sessions/${encodeURIComponent(sessionKey)}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        return;
