...
function getDeckApiBase(): string {
  const raw = process.env.NEXT_PUBLIC_DECK_GO_API_BASE?.trim() ?? "";
  return raw.replace(/\/+$/, "");
}

function resolveDeckInput(input: RequestInfo | URL): RequestInfo | URL {
  const apiBase = getDeckApiBase();
  if (!apiBase) {
    return input;
  }
  if (typeof input === "string") {
    return input.startsWith("/api/") ? `${apiBase}${input}` : input;
  }
  if (input instanceof URL) {
    const asString = input.toString();
    return asString.startsWith("/api/") ? new URL(`${apiBase}${asString}`) : input;
  }
  return input;
}
