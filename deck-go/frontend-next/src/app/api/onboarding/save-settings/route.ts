import { setSetting } from "@server/deck-settings";
import { initRuntime, shutdownRuntime } from "@server/runtime";
/**
 * POST /api/onboarding/save-settings — Persist onboarding configuration.
 *
 * Saves gateway URL/token (and optional provider fields) to the JSON
 * settings store, then triggers runtime initialization.
 *
...
async function localOnboardingSaveSettingsPostHandler(request: NextRequest) {
  ...
  setSetting("gateway_url", url);
  setSetting("gateway_token", token);

  if (body.providerName) {
    setSetting("provider_name", body.providerName);
  }
  if (body.apiKey) {
    setSetting("provider_api_key", body.apiKey);
  }
  if (body.model) {
    setSetting("provider_model", body.model);
  }
...
const guardedLocalOnboardingSaveSettingsPostHandler = withAuth(localOnboardingSaveSettingsPostHandler);
...
export async function POST(request: NextRequest) {
  ...
  return guardedLocalOnboardingSaveSettingsPostHandler(request);
}
