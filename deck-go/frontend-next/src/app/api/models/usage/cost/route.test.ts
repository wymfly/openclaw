...
describe("/api/models/usage/cost", () => {
  ...
  it("proxies GET to deck-go when NEXT_PUBLIC_DECK_GO_API_BASE is set", async () => {
    ...
    const response = await GET(new NextRequest("http://localhost/api/models/usage/cost?days=14"));

    expect(fetchMock).toHaveBeenCalledWith(
      "http://127.0.0.1:19528/api/v1/models/usage/cost?days=14",
  ...
  it("falls back to local gatewayRequest for usage.cost", async () => {
