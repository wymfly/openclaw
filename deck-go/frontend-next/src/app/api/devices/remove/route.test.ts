...
describe("/api/devices/remove", () => {
  ...
  it("proxies POST to deck-go when NEXT_PUBLIC_DECK_GO_API_BASE is set", async () => {
    ...
    const response = await POST(
      new NextRequest("http://localhost/api/devices/remove", {
        method: "POST",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://127.0.0.1:19528/api/v1/runtimes/rt_local/devices/remove",
  ...
  it("falls back to local gwCall when no deck-go base is configured", async () => {
    ...
    expect(gwCall).toHaveBeenCalledWith("device.pair.remove", { deviceId: "dev-1" });
  });
});
