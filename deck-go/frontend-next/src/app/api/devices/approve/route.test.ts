...
describe("/api/devices/approve", () => {
  ...
  it("proxies POST to deck-go when NEXT_PUBLIC_DECK_GO_API_BASE is set", async () => {
    ...
    const response = await POST(
      new NextRequest("http://localhost/api/devices/approve", {
        method: "POST",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://127.0.0.1:19528/api/v1/runtimes/rt_local/devices/approve",
  ...
  it("falls back to local gwCall when no deck-go base is configured", async () => {
    ...
    expect(gwCall).toHaveBeenCalledWith("device.pair.approve", { requestId: "req-1" });
  });
});
