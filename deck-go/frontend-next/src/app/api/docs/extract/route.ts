...
async function localDocsExtractPostHandler(request: NextRequest) {
...
const guardedLocalDocsExtractPostHandler = withAuth(localDocsExtractPostHandler);

export async function POST(request: NextRequest) {
  const proxied = await maybeProxyToDeckGo(request, "/api/v1/docs/extract");
  if (proxied) {
    return proxied;
  }
  return guardedLocalDocsExtractPostHandler(request);
}
