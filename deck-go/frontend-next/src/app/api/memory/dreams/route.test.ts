...
describe("/api/memory/dreams", () => {
  ...
  it("proxies POST to deck-go when NEXT_PUBLIC_DECK_GO_API_BASE is set", async () => {
    ...
    const response = await POST(
      new NextRequest("http://localhost/api/memory/dreams", {
        method: "POST",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://127.0.0.1:19528/api/v1/runtimes/rt_local/memory/dreams",
  ...
  it("falls back to local gwRequest when no deck-go base is configured", async () => {
    ...
    expect(gwRequest).toHaveBeenCalledWith("doctor.memory.dreamDiary", {});
  });
});
