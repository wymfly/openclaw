...
async function localMemoryBrowseGetHandler(request: NextRequest) {
...
const guardedLocalMemoryBrowseGetHandler = withAuth(localMemoryBrowseGetHandler);

export async function GET(request: NextRequest) {
  const search = request.nextUrl.search ?? "";
  const proxied = await maybeProxyToDeckGo(request, `/api/v1/memory/browse${search}`);
  if (proxied) {
    return proxied;
  }
  return guardedLocalMemoryBrowseGetHandler(request);
}
