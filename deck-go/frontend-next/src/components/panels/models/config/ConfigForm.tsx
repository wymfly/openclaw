...
export function ConfigForm({ provider, initialConfig, authType, onSave }: ConfigFormProps) {
  ...
  const [localModels, setLocalModels] = useState(initialConfig?.models ?? []);
  ...
  return (
    <Card className="transition-panel">
      ...
      <CardContent className="space-y-4 pt-4">
        <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--primary-muted)]/40 px-3 py-2 text-xs text-[var(--muted-foreground)]">
          {t("config.formScopeHint")}
        </div>
