"use client";

import { useTranslations } from "next-intl";
import type { WebhookDelivery } from "@/stores/webhooks";

interface DeliveryHistoryProps {
  deliveries: WebhookDelivery[];
  onTest: () => void;
  testing?: boolean;
}

export function DeliveryHistory({ deliveries, onTest, testing }: DeliveryHistoryProps) {
  const t = useTranslations("webhooks");

  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium" style={{ color: "var(--foreground)" }}>
          {t("deliveries")}
        </h3>
        <button
          type="button"
          className="px-3 py-1 text-xs rounded-md font-medium transition-colors"
          style={{ backgroundColor: "var(--primary)", color: "var(--primary-foreground)" }}
          onClick={onTest}
          disabled={testing}
        >
          {testing ? "..." : t("testDelivery")}
        </button>
      </div>

      {deliveries.length === 0 ? (
        <p className="text-xs py-4 text-center" style={{ color: "var(--muted-foreground)" }}>
          {t("noDeliveries")}
        </p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr
                className="border-b"
                style={{ borderColor: "var(--border)", color: "var(--muted-foreground)" }}
              >
                <th className="text-left py-2 px-2 font-medium">{t("time")}</th>
                <th className="text-left py-2 px-2 font-medium">{t("eventType")}</th>
                <th className="text-left py-2 px-2 font-medium">{t("statusCode")}</th>
                <th className="text-left py-2 px-2 font-medium">{t("responseTime")}</th>
                <th className="text-left py-2 px-2 font-medium">{t("status")}</th>
              </tr>
            </thead>
            <tbody>
              {deliveries.map((d) => {
                const detail = d.error ?? d.responseBody ?? null;
                return (
                  <tr key={d.id} className="border-b" style={{ borderColor: "var(--border)" }}>
                    <td className="py-2 px-2 align-top" style={{ color: "var(--muted-foreground)" }}>
                      {new Date(d.createdAt).toLocaleString()}
                    </td>
                    <td className="py-2 px-2 align-top" style={{ color: "var(--foreground)" }}>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span>{d.eventType}</span>
                        {d.isRetry && (
                          <span
                            className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium"
                            style={{
                              backgroundColor: "var(--warning-muted)",
                              color: "var(--warning)",
                            }}
                          >
                            {t("retrying")}
                          </span>
                        )}
                        {typeof d.attempt === "number" && d.attempt > 0 && (
                          <span
                            className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium"
                            style={{
                              backgroundColor: "var(--neutral-muted)",
                              color: "var(--neutral-muted-text)",
                            }}
                          >
                            {t("attempt")} {d.attempt}
                          </span>
                        )}
                      </div>
                      {detail && (
                        <p className="mt-1 text-[11px] break-words" style={{ color: "var(--muted-foreground)" }}>
                          <span className="font-medium">{t("details")}:</span> {detail}
                        </p>
                      )}
                    </td>
                    <td className="py-2 px-2 align-top" style={{ color: "var(--foreground)" }}>
                      {d.statusCode ?? "—"}
                    </td>
                    <td className="py-2 px-2 align-top" style={{ color: "var(--muted-foreground)" }}>
                      {d.durationMs != null ? `${d.durationMs}ms` : "—"}
                    </td>
                    <td className="py-2 px-2 align-top">
                      <span
                        className="inline-block px-2 py-0.5 rounded-full text-xs font-medium"
                        style={{
                          backgroundColor: d.success
                            ? "var(--success-muted)"
                            : "var(--destructive-muted)",
                          color: d.success ? "var(--success)" : "var(--destructive)",
                        }}
                      >
                        {d.success ? t("success") : t("failed")}
                      </span>
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}
