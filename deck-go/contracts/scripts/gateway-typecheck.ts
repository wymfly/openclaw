...
function scanFrontend(): Finding[] {
  ...
  const ambiguousPaths = new Set(
    [...categoriesByPath.entries()]
      .filter(([, categories]) => categories.has("gateway-rpc-proxy") && categories.size > 1)
      .map(([path]) => path),
  );
  const gatewayProxyPatterns = endpoints
    .filter((endpoint) => endpoint.category === "gateway-rpc-proxy")
...
function writeExceptions(exceptions: Finding[]): void {
...
function main(): void {
  ...
  const violations = [...go.violations, ...feViolations];
  writeExceptions(go.exceptions);
