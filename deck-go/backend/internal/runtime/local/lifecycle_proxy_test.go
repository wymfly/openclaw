...
func TestCLIServiceQuerier_ParsesJSONAfterNodeWarning(t *testing.T) {
	exec := &fakeExec{
		out: []byte(`(node:123) Warning: The 'NO_COLOR' env is ignored due to the 'FORCE_COLOR' env being set.
{"service":{"loaded":true,"runtime":{"status":"running"}}}`),
	}
	state, err := (&CLIServiceQuerier{Proxy: newProxy(exec)}).Query(context.Background(), "ignored")
	if err != nil {
		t.Fatalf("Query() error = %v", err)
	}
	if !state.Registered || !state.Active {
		t.Fatalf("Query() = %#v, want registered active service", state)
	}
}
