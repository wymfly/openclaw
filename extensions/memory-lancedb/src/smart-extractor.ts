...
import type { MemoryEntry, MemorySearchResult } from "./store-types.js";
import { isUserMdExclusiveMemory, type WorkspaceBoundaryConfig } from "./workspace-boundary.js";

// ============================================================================
// Extended Store interface for smart-extractor
// ============================================================================

/**
 * Extended MemoryStore interface with write operations needed by the extractor.
 * The base MemoryStore in store-types.ts is read-only for retriever use.
 */
...
export class SmartExtractor {
  ...
  async extractAndPersist(
    ...
    const scopeFilter =
      options.scopeFilter && options.scopeFilter.length > 0 ? options.scopeFilter : [targetScope];

    // Step 1: LLM extraction
    const candidates = await this.extractCandidates(conversationText);

    if (candidates.length === 0) {
      this.log("memory-lancedb: smart-extractor: no memories extracted");
      this.learnAsNoise(conversationText);
      return stats;
    }

    this.log(`memory-lancedb: smart-extractor: extracted ${candidates.length} candidate(s)`);
    ...
    for (const candidate of candidates.slice(0, MAX_MEMORIES_PER_EXTRACTION)) {
      if (
        ...
        this.log(
          `memory-lancedb: smart-extractor: skipped USER.md-exclusive [${candidate.category}] ${candidate.abstract.slice(0, 60)}`,
        );
        continue;
      }
      ...
      try {
        ...
        this.log(
          `memory-lancedb: smart-extractor: failed to process candidate [${candidate.category}]: ${String(err)}`,
        );
      }
    }
    ...
    for (const text of texts) {
      ...
      try {
        ...
        if (!vec || vec.length === 0 || !noiseBank.isNoise(vec)) {
          ...
          this.debugLog(
            `memory-lancedb: smart-extractor: embedding noise filtered: ${text.slice(0, 80)}`,
          );
        }
      } catch {
        result.push(text);
  ...
  private async learnAsNoise(conversationText: string): Promise<void> {
    ...
    try {
      ...
      if (vec && vec.length > 0) {
        noiseBank.learn(vec);
        this.debugLog("memory-lancedb: smart-extractor: learned noise from zero-extraction");
      }
    } catch {
  ...
  private async extractCandidates(conversationText: string): Promise<CandidateMemory[]> {
    ...
    if (!result) {
      this.debugLog("memory-lancedb: smart-extractor: extract-candidates returned null");
      return [];
    }
    if (!result.memories || !Array.isArray(result.memories)) {
      this.debugLog(
        `memory-lancedb: smart-extractor: extract-candidates returned unexpected shape keys=${Object.keys(result).join(",") || "(none)"}`,
      );
      return [];
    }

    this.debugLog(
      `memory-lancedb: smart-extractor: extract-candidates raw memories=${result.memories.length}`,
    );
    ...
    for (const raw of result.memories) {
      ...
      if (!category) {
        ...
        this.debugLog(
          `memory-lancedb: smart-extractor: dropping candidate due to invalid category rawCategory=${JSON.stringify(raw.category ?? "")} abstract=${JSON.stringify((raw.abstract ?? "").trim().slice(0, 120))}`,
        );
        continue;
      }
      ...
      if (!abstract || abstract.length < 5) {
        shortAbstractCount++;
        this.debugLog(
          `memory-lancedb: smart-extractor: dropping candidate due to short abstract category=${category} abstract=${JSON.stringify(abstract)}`,
        );
        continue;
      }
      if (isNoise(abstract)) {
        noiseAbstractCount++;
        this.debugLog(
          `memory-lancedb: smart-extractor: dropping candidate due to noise abstract category=${category} abstract=${JSON.stringify(abstract.slice(0, 120))}`,
        );
        continue;
      }
    ...
    this.debugLog(
      `memory-lancedb: smart-extractor: validation summary accepted=${candidates.length}, invalidCategory=${invalidCategoryCount}, shortAbstract=${shortAbstractCount}, noiseAbstract=${noiseAbstractCount}`,
    );
  ...
  private async processCandidate(
    ...
    if (!vector || vector.length === 0) {
      this.log("memory-lancedb: smart-extractor: embedding failed, storing as-is");
    ...
    switch (dedupResult.decision) {
      ...
      case "skip":
        this.log(
          `memory-lancedb: smart-extractor: skipped [${candidate.category}] ${candidate.abstract.slice(0, 60)}`,
        );
        stats.skipped++;
        break;
  ...
  private async llmDedupDecision(
    ...
    try {
      ...
      if (!data) {
        this.log(
          "memory-lancedb: smart-extractor: dedup LLM returned unparseable response, defaulting to CREATE",
        );
      ...
      if (destructiveDecisions.has(decision) && !hasValidIndex) {
        this.log(
          `memory-lancedb: smart-extractor: ${decision} decision has missing/invalid match_index (${idx}), degrading to create`,
        );
        return {
          decision: "create",
      ...
      this.log(`memory-lancedb: smart-extractor: dedup LLM failed: ${String(err)}`);
  ...
  private async handleMerge(
    ...
    try {
      ...
      this.log(
        `memory-lancedb: smart-extractor: could not read existing memory ${matchId}, storing as new`,
      );
    ...
    if (!merged) {
      this.log("memory-lancedb: smart-extractor: merge LLM failed, skipping merge");
      return;
    }
    ...
    this.log(
      `memory-lancedb: smart-extractor: merged [${candidate.category}]${contextLabel ? ` [${contextLabel}]` : ""} into ${matchId.slice(0, 8)}`,
    );
  }
  ...
  private async handleSupersede(
    ...
    this.log(
      `memory-lancedb: smart-extractor: superseded [${candidate.category}] ${matchId.slice(0, 8)} -> ${created.id.slice(0, 8)}`,
    );
  }
  ...
  private async handleSupport(
    matchId: string,
    scopeFilter: string[],
    _source: { session: string; timestamp: number },
    reason: string,
    contextLabel?: string,
  ): Promise<void> {
    const existing = await this.store.getById(matchId, scopeFilter);
    ...
    this.log(
      `memory-lancedb: smart-extractor: support [${contextLabel || "general"}] on ${matchId.slice(0, 8)} — ${reason}`,
    );
  }
  ...
  private async handleContextualize(
    ...
    this.log(
      `memory-lancedb: smart-extractor: contextualize [${contextLabel || "general"}] new entry linked to ${matchId.slice(0, 8)}`,
    );
  }
  ...
  private async handleContradict(
    ...
    this.log(
      `memory-lancedb: smart-extractor: contradict [${contextLabel || "general"}] on ${matchId.slice(0, 8)}, new entry created`,
    );
  }
  ...
  private async storeCandidate(
    ...
    this.log(
      `memory-lancedb: smart-extractor: created [${candidate.category}] ${candidate.abstract.slice(0, 60)}`,
    );
  }
