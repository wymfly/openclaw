...
function getErrorStatus(error: unknown): number | undefined {
  ...
  if (err.error && typeof err.error === "object") {
    ...
    if (typeof nested.statusCode === "number") return nested.statusCode as number;
  }
  return undefined;
}
...
export class Embedder {
  ...
  constructor(config: EmbeddingConfig & { chunking?: boolean }) {
    ...
    if (this.clients.length > 1) {
      console.log(
        `[memory-lancedb] Initialized ${this.clients.length} API keys for round-robin rotation`,
      );
    }
  ...
  private isRateLimitError(error: unknown): boolean {
    ...
    if (nested && typeof nested === "object") {
      ...
      if (n.type === "rate_limit_exceeded" || n.type === "insufficient_quota") return true;
      if (n.code === "rate_limit_exceeded" || n.code === "insufficient_quota") return true;
    }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (this.isRateLimitError(error) && attempt < maxAttempts - 1) {
          console.log(
            `[memory-lancedb] Attempt ${attempt + 1}/${maxAttempts} hit rate limit, rotating to next key...`,
          );
          continue;
        }
        ...
        try {
          ...
          const chunkEmbeddings = await Promise.all(
            chunkResult.chunks.map(async (chunk, idx) => {
              try {
                const emb = await this.embedSingle(chunk, task);
