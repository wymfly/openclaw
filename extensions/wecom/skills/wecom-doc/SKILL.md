...
读取 wecom 运行时配置文件，该文件由 wecom channel 长连接建立时写入（`{accountId}` 默认为 `"default"`，多账户时为具体账户 ID）：

```bash
cat ~/.openclaw/wecomConfig/{accountId}/config.json
```
...
```bash
openclaw config get channels.wecom.bot.ws.botId 2>&1
```

> 如果使用 flat-key 配置（`channels.wecom.botId`），系统会通过兼容适配器自动映射。
