...
describe("wecom agent uploadMedia", () => {
  ...
  it("uses text/plain for .txt uploads", async () => {
    wecomFetchMock.mockResolvedValueOnce(
      jsonResponse({ access_token: "token-1", expires_in: 7200 }),
    );
    wecomFetchMock.mockResolvedValueOnce(
      jsonResponse({ errcode: 0, errmsg: "ok", media_id: "m-1" }),
    );
  ...
  it("uses docx mime and normalizes non-ascii filename", async () => {
    ...
    wecomFetchMock.mockResolvedValueOnce(
      jsonResponse({ errcode: 0, errmsg: "ok", media_id: "m-2" }),
    );
  ...
  it("retries with octet-stream when preferred mime upload fails", async () => {
    wecomFetchMock.mockResolvedValueOnce(
      jsonResponse({ access_token: "token-3", expires_in: 7200 }),
    );
    wecomFetchMock.mockResolvedValueOnce(
      jsonResponse({ errcode: 40005, errmsg: "invalid media type" }),
    );
    wecomFetchMock.mockResolvedValueOnce(
      jsonResponse({ errcode: 0, errmsg: "ok", media_id: "m-3" }),
    );
