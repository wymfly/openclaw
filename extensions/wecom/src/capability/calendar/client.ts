...
import type {
  ...
  AddScheduleAttendeesRequest,
  AddScheduleAttendeesResponse,
  DeleteScheduleAttendeesRequest,
  DeleteScheduleAttendeesResponse,
  GetScheduleByCalendarRequest,
  GetScheduleByCalendarResponse,
  GetScheduleRequest,
  GetScheduleResponse,
  DeleteScheduleRequest,
  DeleteScheduleResponse,
  GetSystemCalendarIdRequest,
  GetSystemCalendarIdResponse,
  CreateSystemScheduleRequest,
  RespondScheduleRequest,
  RespondScheduleResponse,
  SyncScheduleRequest,
...
export class WecomCalendarClient {
  ...
  async updateSchedule(p: {
  ...
  }): Promise<{ raw: UpdateScheduleResponse; scheduleId: string }> {
    const schedule = p.request.schedule;

    const scheduleId = validateString(schedule.schedule_id, "schedule.schedule_id", {
      allowEmpty: false,
    });
    const startTime = validateNumber(schedule.start_time, "schedule.start_time", {
      min: 0,
    ...
    const opStartTime =
      p.request.op_start_time !== undefined
    ...
    const json = await this.post<UpdateScheduleResponse>(
  ...
  async addScheduleAttendees(p: {
    agent: ResolvedAgentAccount;
    request: AddScheduleAttendeesRequest;
  }): Promise<{ raw: AddScheduleAttendeesResponse; scheduleId: string }> {
    ...
    const json = await this.post<AddScheduleAttendeesResponse>(
  ...
  async deleteScheduleAttendees(p: {
    agent: ResolvedAgentAccount;
    request: DeleteScheduleAttendeesRequest;
  }): Promise<{ raw: DeleteScheduleAttendeesResponse; scheduleId: string }> {
    const scheduleId = validateString(p.request.schedule_id, "schedule_id", { allowEmpty: false });
    const attendees = validateScheduleAttendees(p.request.attendees);
    ...
    const json = await this.post<DeleteScheduleAttendeesResponse>(
  ...
  async getScheduleByCalendar(p: {
    agent: ResolvedAgentAccount;
    request: GetScheduleByCalendarRequest;
  }): Promise<{ raw: GetScheduleByCalendarResponse; scheduleList: ScheduleInfo[] }> {
    ...
    if (limit !== undefined) {
      if (
        limit < CALENDAR_LIMITS.GET_SCHEDULE_LIMIT_MIN ||
        limit > CALENDAR_LIMITS.GET_SCHEDULE_LIMIT_MAX
      ) {
        throw new Error(
          `limit 必须在 ${CALENDAR_LIMITS.GET_SCHEDULE_LIMIT_MIN}-${CALENDAR_LIMITS.GET_SCHEDULE_LIMIT_MAX} 之间`,
        );
      }
    }
    ...
    const json = await this.post<GetScheduleByCalendarResponse>(
   ...
   */
  async getSchedule(p: { agent: ResolvedAgentAccount; request: GetScheduleRequest }): Promise<{
    raw: GetScheduleResponse;
    ...
    const json = await this.post<GetScheduleResponse>(
  ...
  async deleteSchedule(p: {
  ...
  }): Promise<{ raw: DeleteScheduleResponse; scheduleId: string }> {
    const scheduleId = validateString(p.request.schedule_id, "schedule_id", { allowEmpty: false });
    const opMode = validateOpMode(p.request.op_mode);
    const opStartTime =
      p.request.op_start_time !== undefined
    ...
    const json = await this.post<DeleteScheduleResponse>(
  ...
  }): Promise<{ raw: RespondScheduleResponse; scheduleId: string }> {
    const scheduleId = validateString(p.request.schedule_id, "schedule_id", { allowEmpty: false });
    const opMode = validateOpMode(p.request.op_mode);
    const opStartTime =
      p.request.op_start_time !== undefined
  ...
  async syncSchedule(p: {
  ...
  }): Promise<{ raw: SyncScheduleResponse; nextCursor: string; scheduleList: ScheduleInfo[] }> {
    const calId = validateString(p.request.cal_id, "cal_id", { allowEmpty: false });
    ...
    const json = await this.post<SyncScheduleResponse>(
      "/cgi-bin/oa/schedule/sync",
      "sync_schedule",
      p.agent,
      request,
    );
