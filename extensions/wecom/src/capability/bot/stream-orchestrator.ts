...
export function createBotStreamOrchestrator(params: {
  ...
  async function startAgentForStream(params: StartBotAgentStreamParams): Promise<void> {
    ...
    if (params.mergedContents) {
      rawBody = Array.isArray(params.mergedContents)
    ...
    if (
      shouldRejectWecomDefaultRoute({ cfg: config, matchedBy: route.matchedBy, useDynamicAgent })
    ) {
      const prompt =
      ...
      try {
        ...
        target.runtime.error?.(
          `routing guard prompt push failed streamId=${streamId}: ${String(err)}`,
        );
      }
      streamStore.onStreamFinished(streamId);
      return;
    ...
    logVerbose(
      target,
      "tool-policy: WeCom Bot 会话已禁用 message 工具（tools.deny += message；并同步到 tools.sandbox.tools.deny，防止绕过 Bot 交付）",
    );
