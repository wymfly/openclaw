        ...
        try {
          ...
          if (agentCfg && current.userId && !current.agentMediaKeys.includes(mediaPath)) {
            try {
              ...
              logVerbose(
                target,
                `fallback(error): 媒体处理失败后已通过 Agent 私信发送 user=${current.userId}`,
              );
            } catch (sendErr) {
              target.runtime.error?.(
                `fallback(error): 媒体处理失败后的 Agent 私信发送也失败: ${String(sendErr)}`,
              );
              recordBotOperationalEvent(target, {
                category: "fallback-delivery-failed",
          ...
          if (!current.fallbackMode) {
            ...
            try {
              ...
              target.runtime.error?.(
                `wecom bot fallback prompt push failed (error) streamId=${streamId}: ${String(pushErr)}`,
              );
              recordBotOperationalEvent(target, {
                category: "fallback-delivery-failed",
