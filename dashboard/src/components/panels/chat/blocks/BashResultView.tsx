...
export function BashResultView({ result }: BashResultViewProps) {
  ...
  return (
    <div className="my-1.5 text-xs rounded-lg border border-[var(--border-subtle)] overflow-hidden">
      {/* Command header bar */}
      <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-[var(--bg-tertiary)] text-[var(--text-secondary)] border-b border-[var(--border-subtle)]">
        <Terminal size={12} className="shrink-0" />
        <span className="font-medium">{t("bashCommand")}:</span>
        <code className="font-mono text-[var(--accent)] truncate">$ {result.command}</code>
        {/* Exit code badge */}
        <span
      ...
      {result.stdout && (
        <div className="px-2.5 py-2">
          ...
          <pre className="text-xs whitespace-pre-wrap overflow-auto font-mono text-[var(--text-primary)] max-h-[300px]">
            {result.stdout}
          </pre>
        </div>
      )}
      ...
      {result.stderr && (
        <div className="px-2.5 py-2 bg-[var(--danger-muted)] border-t border-[var(--border-subtle)]">
          <div className="text-[10px] font-medium text-[var(--danger-muted-text)] mb-1 uppercase tracking-wide">
            {t("bashStderr")}
          </div>
          <pre className="text-xs whitespace-pre-wrap overflow-auto font-mono text-[var(--danger-muted-text)] max-h-[200px]">
            {result.stderr}
          </pre>
        </div>
      )}
