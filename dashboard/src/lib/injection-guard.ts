/**
 * Injection Guard — prompt injection and command injection detection.
 *
 * Transplanted from Mission Control, adapted for openclaw-deck:
 * - Removed dynamic require of security-events (logging is optional)
 * - All 16 detection regex rules preserved
 * - Three protection layers: prompt injection, command injection, exfiltration
 *
...
const RULES: InjectionRule[] = [
  // -- Prompt injection: system override --
  {
    ...
    severity: "critical",
    pattern:
      /\b(?:ignore|disregard|forget|override)\s+(?:all\s+)?(?:previous|prior|above|your|system)\s+(?:instructions?|rules?|guidelines?|prompts?|directives?|constraints?)/i,
  ...
  },
  {
    ...
    severity: "critical",
    pattern:
      /\b(?:you\s+are\s+now|act\s+as\s+(?:if\s+you\s+(?:are|were)\s+)?(?:a\s+)?(?:(?:un)?restricted|evil|jailbr(?:o|ea)ken|different|new))\b/i,
  ...
  },
  {
    ...
    severity: "critical",
    pattern:
      /\b(?:bypass|disable|turn\s+off|deactivate|circumvent)\s+(?:all\s+)?(?:safety|security|content|moderation|ethic(?:al|s)?)\s*(?:filters?|checks?|guard(?:rail)?s?|rules?|measures?|restrictions?)?\b/i,
  ...
  },
  {
    ...
    severity: "critical",
    pattern:
      /\[(?:SYSTEM|INST|HIDDEN|ADMIN|IMPORTANT)\s*(?:OVERRIDE|MESSAGE|INSTRUCTION)?[\]:]\s*.{10,}/i,
  ...
  },
  {
    ...
    severity: "warning",
    pattern:
      /(?:<\/?(?:system|user|assistant|human|ai|instruction|context)>|```\s*system\b|\|>\s*(?:system|admin)\b)/i,
  ...
  },
  {
    ...
    severity: "warning",
    pattern:
      /\b(?:repeat|recite|echo|output|print|reveal|show|display)\s+(?:your\s+)?(?:system\s+)?(?:prompt|instructions?|rules?|guidelines?|initial\s+(?:message|prompt))\b/i,
  ...
  // -- Command injection --
  {
    ...
    severity: "critical",
    pattern:
      /(?:[;&|`$]\s*(?:rm\b|wget\b|curl\b|nc\b|ncat\b|bash\b|sh\b|python\b|perl\b|ruby\b|php\b|node\b))|(?:\$\(.*(?:rm|wget|curl|nc|bash|sh))/i,
  ...
  },
  {
    ...
    severity: "critical",
    pattern:
      /\b(?:\/dev\/tcp\/|mkfifo|nc\s+-[elp]|ncat\s.*-[elp]|bash\s+-i\s+>&?\s*\/dev\/|python.*socket.*connect)\b/i,
  ...
  },
  {
    ...
    severity: "warning",
    pattern:
      /\b(?:printenv|env\b.*(?:AUTH_PASS|API_KEY|SECRET|TOKEN)|cat\s+(?:\/proc\/self\/environ|\.env\b|\/etc\/(?:shadow|passwd)))/i,
  ...
  // -- Template injection --
  {
    ...
    severity: "warning",
    pattern:
      /\{\{.*(?:config|settings|env|self|request|__class__|__globals__|__builtins__).*\}\}|<%.*(?:Runtime|Process|exec|system|eval).*%>|\$\{.*(?:Runtime|exec|java\.lang).*\}/i,
  ...
  // -- SQL injection --
  {
    ...
    severity: "critical",
    pattern:
      /(?:\bUNION\s+(?:ALL\s+)?SELECT\b|\b;\s*DROP\s+TABLE\b|'\s*OR\s+['"]?1['"]?\s*=\s*['"]?1|'\s*;\s*(?:DELETE|INSERT|UPDATE|ALTER)\s)/i,
  ...
  // -- Exfiltration --
  {
    ...
    severity: "critical",
    pattern:
      /\b(?:send|post|upload|transmit|exfiltrate|forward)\s+(?:all\s+)?(?:the\s+)?(?:data|files?|contents?|secrets?|keys?|tokens?|credentials?|passwords?|env(?:ironment)?)\s+(?:to|via|using|through)\b/i,
...
export function sanitizeForShell(input: string): string {
  // Remove null bytes using split/join to avoid control-regex lint rule
  return input
